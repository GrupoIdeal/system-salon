...
export async function addToWaitlist(
  ...
  if (process.env.NODE_ENV !== "production") {
    logger.info(`Cliente adicionado a lista de espera:`, {
      id: newEntry.id,
...
export async function removeFromWaitlist(waitlistId: string): Promise<boolean> {
  ...
  if (removed) {
    if (process.env.NODE_ENV !== "production") {
      logger.info(`Removido da lista de espera: ${waitlistId}`);
    }
  }
...
export async function notifyWaitlistClient(
  ...
  try {
    ...
    if (process.env.NODE_ENV !== "production") {
      logger.info(`Notificando cliente da lista de espera:`, {
    ...
    logger.error("Erro ao notificar cliente da lista de espera:", error);
    return false;
  }
}
...
export async function confirmWaitlistSlot(
  ...
  if (process.env.NODE_ENV !== "production") {
    logger.info(`Horario da lista de espera confirmado:`, {
      waitlistId,
      appointmentId,
...
export async function updateWaitlistEntry(
  ...
  logger.info(`Lista de espera atualizada:`, {
    waitlistId,
    updates,
  });
