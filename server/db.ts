...
export async function getAppointmentsWithDetailsBySalonId(
  ...
  return result.map(row => ({
    ...
    deletedAt: null,
    client: { ...row.client, deletedAt: null },
    service: { ...row.service, deletedAt: null },
    specialist: { ...row.specialist, deletedAt: null },
  }));
}
...
export async function recordAppointmentRevenue(
  ...
  if (typeof amountPaid === "number" && !Number.isNaN(amountPaid)) {
    try {
      ...
      logger.error("Failed to save paidAmount on appointment:", err);
    }
  }
...
export async function validateAppointmentSlot(
  ...
  logger.debug("validateAppointmentSlot DEBUG:", {
    specialistId,
    serviceId,
  ...
  logger.info("Day validation info:", {
    dayOfWeek,
    dayName,
  ...
  try {
    ...
    if (needsSync) {
      logger.info("Auto-syncing legacy workingDays to schedule system");
    ...
    if (
      ...
      logger.info("Using specialist schedule system for validation");
    } else {
      ...
      logger.warn("Using legacy workingDays system for validation");
    }
  } catch (error) {
    logger.warn(
      "Error with schedule system, falling back to legacy:",
      error
    );
    // Fallback para o sistema antigo
    if (!specialist.workingDays?.[dayName]) {
...
export async function removeTemporaryBlock(blockId: string): Promise<void> {
  ...
  logger.info("Removendo bloqueio temporário:", blockId);
}
...
export async function getAvailableTimeSlots(
  ...
  logger.debug("getAvailableTimeSlots DEBUG:", {
    specialistId,
    serviceId,
    date: date.toISOString(),
  ...
  if (!specialist) {
    logger.error("Specialist not found");
    return [];
  }
  ...
  if (service.specialistId && service.specialistId !== specialistId) {
    logger.error(
      ...
      { serviceSpecialistId: service.specialistId, specialistId }
    );
    return [];
  }
  ...
  try {
    ...
    const isWorking = await isSpecialistWorking(specialistId, date);
    if (!isWorking) {
      logger.error("Specialist does not work on this day (schedule system)");
      return [];
    }
    ...
    if (
      ...
      logger.info("Using specialist schedule system");
    } else {
      // Fallback para o sistema antigo
      if (!specialist.workingDays?.[dayName]) {
        logger.error("Specialist does not work on this day");
        return [];
      }
      specialistPeriods = specialist.workingDays[dayName];
      logger.warn("Using legacy workingDays system");
    }
  } catch (error) {
    logger.warn(
      "Error with schedule system, falling back to legacy:",
  ...
  logger.info("Specialist working periods:", specialistPeriods);

  // Gera todos os slots possíveis para cada período do especialista
  for (const period of specialistPeriods) {
    logger.info("Generating slots for period:", period);
    const periodSlots = generateTimeSlots(period.start, period.end, 30); // Slots de 30 em 30 minutos
    logger.info("Generated slots:", { count: periodSlots.length });
  ...
  logger.info("Total slots before conflict check:", { count: allSlots.length });
  ...
  logger.info("Existing appointments:", {
  ...
  const availableSlots = allSlots.filter(slot => {
    ...
    const hasConflict = existingAppointments.some(apt => {
      ...
      if (!apt.appointmentTime || typeof apt.appointmentTime !== "string") {
        logger.warn("Skipping appointment with invalid time:", {
          id: apt.id,
      ...
      if (conflict) {
        logger.warn(
          "Conflict detected:",
          { slot, slotEndTime, appointmentTime: apt.appointmentTime, aptEndTime }
        );
      }
  ...
  logger.info(
    ...
    { count: availableSlots.length, slots: availableSlots }
  );
  ...
  const finalSlots = isToday
    ? availableSlots.filter(slot => {
        ...
        if (!isAvailable) {
          logger.info(
            "Filtering out slot - too close to current time",
            { slot, currentBrazilTime, minimumAdvanceMinutes }
          );
        }
  ...
  logger.info(
    ...
    { count: finalSlots.length, slots: finalSlots }
  );
...
function minutesToTime(minutes: number): string {
  if (typeof minutes !== "number" || Number.isNaN(minutes) || minutes < 0) {
    logger.error("minutesToTime: Invalid minutes parameter:", minutes);
    return "00:00";
  }
...
export function timeToMinutes(time: string): number {
  if (!time || typeof time !== "string") {
    logger.error("timeToMinutes: Invalid time parameter:", time);
    return 0;
  }

  const parts = time.split(":");
  if (parts.length !== 2) {
    logger.error("timeToMinutes: Invalid time format:", time);
    return 0;
  }

  const [hours, minutes] = parts.map(Number);
  if (Number.isNaN(hours) || Number.isNaN(minutes)) {
    logger.error("timeToMinutes: Non-numeric time parts:", {
      hours,
      minutes,
    });
    return 0;
