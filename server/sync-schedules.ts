...
const logger = createLogger("sync-schedules");
import {
...
export async function syncLegacyWorkingDaysToSchedule(
  ...
  try {
    ...
    if (!specialist || !specialist.workingDays) {
      logger.warn(`Specialist ${specialistId} not found or no workingDays`);
      return;
    }

    logger.info(`Syncing workingDays for specialist ${specialist.name}`);
    ...
    for (const [dayName, periods] of Object.entries(specialist.workingDays)) {
      ...
      if (periods && periods.length > 0) {
        ...
        logger.info(`Set ${getDayName(dayOfWeek)} as non-working day`);
      }
    }

    logger.info(`Sync completed for specialist ${specialist.name}`);
  } catch (error) {
    logger.error(`Error syncing specialist ${specialistId}:`, error);
  }
}
...
export async function syncAllSpecialistsInSalon(
  ...
  try {
    ...
    logger.info(`Sync completed for all specialists in salon ${salonId}`);
  } catch (error) {
    logger.error(`Error syncing salon ${salonId}:`, error);
  }
}
