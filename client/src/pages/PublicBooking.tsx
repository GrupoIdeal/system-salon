...
function PublicBookingPage() {
    const [step, setStep] = useState<'specialist' | 'service' | 'datetime' | 'client' | 'confirmation'>('specialist');
    ...
    const [bookingResult, setBookingResult] = useState<{ success: boolean; message: string; appointmentId?: string } | null>(null);    // Queries tRPC públicas
    ...
    }, [specialistsQuery.status, specialistsQuery.isLoading, specialistsQuery.isError, specialistsQuery.data, specialistsQuery.error]);
    ...
    const createAppointmentMutation = publicTrpc.booking.createPublicAppointment.useMutation({
        onSuccess: (result) => {
    ...
    if (salonQuery.isError || !salonQuery.data || specialistsQuery.isError || !specialistsQuery.data) {
        return (
            <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
                <Card className="w-full max-w-md">
                    <CardContent className="text-center py-8">
                        <h2 className="text-xl font-semibold text-red-600 mb-2">Serviço Indisponível</h2>
                        <p className="text-gray-600">O sistema de agendamento não está disponível no momento.</p>
                    </CardContent>
                </Card>
            </div>
        );
    ...
    const specialists = Array.isArray(specialistsQuery.data) ? specialistsQuery.data : [];
    const services = Array.isArray(servicesQuery.data) ? servicesQuery.data : [];
    const timeSlots = Array.isArray(timeSlotsQuery.data) ? timeSlotsQuery.data : [];

    const selectedSpecialistData = specialists.find((s: SpecialistData) => s.id === selectedSpecialist);
    ...
    const selectedServiceData = services.find((s: ServiceData) => s.id === selectedService);
            ...
            <div className="max-w-4xl mx-auto w-full">
                ...
                <div className="mb-6 px-2 sm:px-0">
                    <div className="flex items-center justify-between text-sm">
                        {['Especialista', 'Serviço', 'Data/Hora', 'Seus Dados', 'Confirmação'].map((stepName, index) => {
                            const stepKeys = ['specialist', 'service', 'datetime', 'client', 'confirmation'];
                            ...
                            return (
                                <div key={stepName} className="flex items-center">
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium ${isCompleted ? 'bg-green-500 text-white' :
                                    ...
                                    {index < 3 && (
                                        <div className={`w-12 h-px mx-4 ${index < currentStepIndex ? 'bg-green-500' : 'bg-gray-200'
                                            }`} />
                                    )}
                                </div>
                            );
                        ...
                        <CardContent>
                            <div className="grid gap-4">
                                ...
                                {!specialistsQuery.isLoading && specialists.length === 0 && (
                                    <div className="text-center py-8">
                                        <p className="text-gray-600">Nenhum especialista disponível no momento.</p>
                                    </div>
                                )}
                                    ...
                                    >
                                        <div className="flex flex-col md:flex-row items-center md:items-start gap-4">
                                            <Avatar className="h-16 w-16 flex-shrink-0">
                                                <AvatarImage className="h-16 w-16 object-cover rounded-full" src={specialist.photo || undefined} alt={specialist.name} />
                                                <AvatarFallback className="h-16 w-16 flex items-center justify-center bg-gray-100 rounded-full"><User className="h-8 w-8 text-gray-500" /></AvatarFallback>
                                            </Avatar>
                                            <div className="flex-1">
                                                ...
                                                {specialist.bio && (
                                                    <p className="text-gray-600 text-sm mt-1">{specialist.bio}</p>
                                                )}
                                            </div>
                                        </div>
                                    </button>
                    ...
                    <Card className="shadow-lg">
                        <CardHeader>
                            ...
                            <div className="text-sm text-gray-600">
                                Serviço: <span className="font-medium">{selectedServiceData.name}</span>
                        ...
                        <CardContent className="text-center py-8">
                            {bookingResult.success ? (
                                <>
                                    ...
                                    <div className="bg-gray-50 rounded-lg p-4 mb-6">
                                        ...
                                        <div className="space-y-1 text-sm text-gray-600">
                                            <div><strong>Especialista:</strong> {selectedSpecialistData?.name}</div>
                                            <div><strong>Serviço:</strong> {selectedServiceData?.name}</div>
                                            <div><strong>Data:</strong> {selectedDate && format(selectedDate, "d 'de' MMMM 'de' yyyy", { locale: ptBR })}</div>
                                    ...
                                    <div className="bg-blue-50 rounded-lg p-4 mb-6">
                                        <p className="text-sm text-blue-800">
                                            <strong>Atenção:</strong> Embora o seu agendamento esteja confirmado, ocasionalmente o atendimento pode atrasar devido a atendimentos anteriores ou imprevistos.
                                            Por favor, planeje uma margem de tempo ao se deslocar até o salão. Agradecemos a compreensão — faremos o possível para minimizar atrasos.
                                        </p>
                                    </div>
                                </>
                            ) : (
                                <>
                                    ...
                                    <h2 className="text-2xl font-bold text-gray-900 mb-2">Erro no Agendamento</h2>
                                    <p className="text-gray-600 mb-6">{bookingResult.message}</p>
                                </>
                            )}
