...
import { Loader2, AlertCircle, User, Calendar, FileText, Clock } from "lucide-react";
...
export function AppointmentModal({
    ...
    const availableSpecialists = specialistsQuery.data?.filter((specialist) => {
        if (!formData.serviceId) return true;
        const service = servicesQuery.data?.find(s => s.id === formData.serviceId);
        return !service?.specialistId || service.specialistId === specialist.id;
    }) || [];

    const selectedClient = clientsQuery.data?.find(c => c.id === formData.clientId);
    const selectedService = servicesQuery.data?.find(s => s.id === formData.serviceId);
    const selectedSpecialist = specialistsQuery.data?.find(s => s.id === formData.specialistId);
    ...
    const validateStep = (currentStep: number): boolean => {
        ...
        switch (currentStep) {
            ...
            case 2:
                ...
                if (!formData.specialistId) newErrors.specialistId = "Selecione um especialista";
                break;
            case 3:
                ...
                return (
                    <div className="space-y-4">
                        ...
                        <CalendarPicker
                            ...
                            onDateSelect={(date) => setFormData({ ...formData, appointmentDate: date })}
                        />
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                                ...
                                <Select
                                    ...
                                    onValueChange={(value) => {
                                        ...
                                        setFormData({
                                            ...
                                            specialistId: service?.specialistId || formData.specialistId
                                        });
                                    }}
                                >
                                    <SelectTrigger>
                            ...
                            <div>
                                            ...
                                            <SelectItem key={specialist.id} value={specialist.id}>
                                                <div className="flex flex-col">
                                                    ...
                                                    {specialist.specialty && (
                                                        <span className="text-xs text-muted-foreground">{specialist.specialty}</span>
                                                    )}
                                                </div>
                                            </SelectItem>
                                        ))}
                                ...
                                {errors.specialistId && (
                                    <p className="text-sm text-destructive mt-1">{errors.specialistId}</p>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="p-4 bg-muted/50 rounded-lg space-y-2">
                            <div className="flex items-center justify-between text-sm">
                                ...
                                <span>{formData.appointmentDate.toLocaleDateString('pt-BR')}</span>
                            </div>
                            <div className="flex items-center justify-between text-sm">
                ...
                return (
                    <div className="space-y-6">
                        ...
                        <div className="space-y-4 p-4 bg-muted/30 rounded-lg border">
                            ...
                            <div>
                                <Label className="text-xs font-medium text-muted-foreground">ESPECIALISTA</Label>
                        ...
                        {appointment && (
                            <div>
                                ...
                                <Select
                                    ...
                                    onValueChange={(value: "pending" | "confirmed" | "completed" | "cancelled") => setFormData({ ...formData, status: value })}
                                >
                                    <SelectTrigger>
                                        <SelectValue />
                                    </SelectTrigger>
